...
class Invoice extends EntityModel implements BalanceAffecting
{
    ...
    // Addition (Dylan) : Return unpaid invoice
    public static function getAllUnpaidInvoices() {
        return Invoice::where('is_deleted', 0)
            ->where('deleted_at', null)
            ->where('balance', '>', 0)
            ->where('invoice_type_id', 1)
            ->where('is_recurring', 0)->get();
    }

    // Addition (Dylan) : Return unpaid invoice (exclude partially paid)
    public static function getFullyUnpaidInvoices() {
        return Invoice::where('is_deleted', 0)
        ->where('deleted_at', null)
        ->where('balance', '>', 0)
        ->whereRaw('invoices.balance = invoices.amount')
        ->where('invoice_type_id', 1)
        ->where('is_recurring', 0)
        ->where('due_date', '>', date('Y-m-d'))->get();
    }

    // Addition (Dylan) : Return late unpaid invoice
    public static function getLateUnpaidInvoices() {
        return Invoice::where('is_deleted', 0)
        ->where('deleted_at', null)
        ->where('balance', '>', 0)
        ->where('invoice_type_id', 1)
    ...
    // Addition (Dylan) : Return partial paid invoice
    public static function getPartiallyPaidInvoices() {
        return Invoice::where('is_deleted', 0)
            ->where('deleted_at', null)
            ->where('balance', '>', 0)
            ->whereRaw('invoices.balance != invoices.amount')
    ...
    // Addition (Dylan) : Return all recurring invoices.
    public static function getAllRecurringInvoices ($year = false) {
        $invoices = Invoice::where('is_recurring', true)->withTrashed();

        if ($year) $invoices->whereDate('start_date', '<', $year.'-12-31');

        return $invoices->get();
    }

    // Addition (Dylan) : Return total amount of recurring invoices for each month of a specific year (+ previous running ones)
    public static function recurringInvoiceStatsData($year) {
        $data = [
            'Jan' => [],
            'Feb' => [],
            'Mar' => [],
        ...
        // Get all recurring invoices starting before the end of the year in parameter
        $invoices = self::getAllRecurringInvoices($year);
        foreach ($invoices as $invoice) {
            $date = Carbon::createFromFormat('Y-m-d', $invoice->start_date);
            $interval = CarbonInterval::make(Frequency::getFrequency($invoice->frequency_id)->date_interval);
            $end_date = $invoice->end_date;
            $deleted_at = $invoice->deleted_at;

            // Loop trough the recurring invoice with $interval step until loop's date become greater than the last day
            // of the $year parameter
            while ($date->lt(Carbon::createFromFormat('Y-m-d', $year.'-12-31'))) {
                // Check if the current loop's date is greater than the end_date + deleted_at date, if so, break the loop
                if ($end_date) {
                    if ($date->gt(Carbon::createFromFormat('Y-m-d', $end_date))) break;
                } else if ($deleted_at) {
                    if ($date->gt($deleted_at)) break;
                }

                // If the invoice pass every if conditions, check if the loop's year (from date) is matching the $year
                // parameter then add to stats array
                $current_month = $data[$date->format('M')];
                if ($date->year == $year) {
                    $items = $invoice->invoice_items;
                    foreach ($items as $item) {
                        $category = explode(' ', $item->product_key)[0];
                        if (!array_key_exists($category, $current_month['categories'])) $current_month['categories'][$category] = 0;
                        $current_month['categories'][$category] += $item->cost*$item->qty;
                        $current_month['amount'] += $item->cost*$item->qty;
                    }
                }
                $data[$date->format('M')] = $current_month;
    ...
    // Addition (Dylan) : Add due date to custom label
    public static function calcStatusLabel($status, $class, $entityType, $quoteInvoiceId, $dueDate = false)
    {
        if ($quoteInvoiceId) {
            $label = 'converted';
        } elseif ($class == 'danger') {
            $label = $entityType == ENTITY_INVOICE ? 'past_due' : 'expired';
            // Addition (Dylan) : Add a new label if client have seen the invoice + it is past due
            if ($label == 'past_due') {
                $label = time() > (strtotime($dueDate) + (60 * 60 * 24)) && $status == 'Viewed' ? 'past_due_and_seen' : 'past_due';
            }
        } else {
    ...
    public function statusLabel()
    {
        // Addition (Dylan) : Add due date to custom label
        $dueDate = $this->getOriginal('partial_due_date') ?: $this->getOriginal('due_date');
        return static::calcStatusLabel($this->invoice_status->name, $this->statusClass(), $this->getEntityType(), $this->quote_invoice_id, $dueDate);
    }
    ...
    public function hidePrivateFields()
    {
        $this->setVisible([
            ...
            'custom_text_value2',
            'has_expenses',
            // Addition (Dylan) : Add parameters to invoice on client portal
            'commitment_period_frequency_id',
            'last_commitment_period_date',
            'next_commitment_period_date',
