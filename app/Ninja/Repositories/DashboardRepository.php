...
{
    public function statsBuilder($startDate, $endDate, ChartService $chartService): stdClass
    {
        $startDate = date_create($startDate);
        $endDate   = date_create($endDate);

        $chartData          = $chartService->getChartData($startDate, $endDate);
        $overviewChart      = $chartService->getOverviewChart($chartData);
        $invoicesChart      = $chartService->getInvoicesChart($chartData);
        $categoriesChart    = $chartService->getCategoriesChart($chartData);
        $yearlySummaryChart = $chartService->getYearlySummary($chartData);
        $paymentsChart      = $chartService->getPaymentsChart($chartData);
        $clientsChart       = $chartService->getClientsChart($chartData);
        $revenuesChart      = $chartService->getRevenuesChart($chartData);
        $revenuesClChart    = $chartService->getRevenuesClChart($chartData);
        $totals             = $chartService->calcTotals($chartData);

        $response = new stdClass();
        $response->overviewChart = $overviewChart;
        $response->invoicesChart = $invoicesChart;
        $response->categoriesChart = $categoriesChart;
        $response->yearlySummaryChart = $yearlySummaryChart;
        $response->paymentsChart = $paymentsChart;
        $response->clientsChart  = $clientsChart;
        $response->revenuesChart  = $revenuesChart;
        $response->revenuesClChart  = $revenuesClChart;
    ...
    /**
     * Addition (Dylan) : Get recurring invoices between 2 dates
     *
     * @return array
     */
    public function upcomingRecurring(): array
    {
        $result     = [];
        $total      = 0;
        $date       = new DateTime('tomorrow');
        $start_date = date_create($date->format('Y-m-d'));
        $end_date   = date_create(date('Y-m-d',strtotime("last day of +1 month")));

        $invoices = Invoice::select(
                'invoices.amount',
                'invoices.last_sent_date',
                'invoices.start_date',
                'invoices.frequency_id',
            ...
            ->where('invoices.is_recurring', 1)
            ->get();

        $collection = new Collection();
        foreach ($invoices as $invoice) {
            $next_date = $invoice->getNextSendDate();
            if($start_date < $next_date && $next_date <= $end_date) {
