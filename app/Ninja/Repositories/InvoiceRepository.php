...
class InvoiceRepository extends BaseRepository
{
    ...
    public function getInvoices($accountId, $clientPublicId = false, $entityType = ENTITY_INVOICE, $filter = false)
    {
        $query = DB::table('invoices')
            ...
            ->join('invoice_items', 'invoice_items.invoice_id', '=', 'invoices.id') // Addition (Dylan) : Join invoice items table
            ...
            ->select(
                ...
                // Addition (Dylan) : Group invoice items label
                DB::raw('GROUP_CONCAT(invoice_items.product_key separator "; ") AS invoice_product_key'),
                'contacts.first_name',
                'contacts.last_name',
                'contacts.email',
                'invoices.quote_id',
        ...
        if ($filter) {
            $query->where(function ($query) use ($filter) {
                $query->where('clients.name', 'like', '%'.$filter.'%')
                      ...
                      ->orWhere('invoice_items.product_key', 'like', '%'.$filter.'%'); // Addition (Dylan) : Add possibility to find invoice by its items
            });
        }
    ...
    public function getRecurringInvoices($accountId, $clientPublicId = false, $filter = false)
    {
        $query = DB::table('invoices')
                    ...
                    ->select(
                        ...
                        'invoices.public_id',
                        'invoices.amount',
                        'frequencies.date_interval as date_interval', // Add frequency date_interval to calculate the new next_due_date field in datatable
                        ...
                        'invoice_statuses.name as status',
                        // Addition (Dylan) : Group invoice items label
                        DB::raw('group_concat(distinct invoice_items.product_key separator "; ") AS invoice_product_key'),
    ...
    public function save(array $data, Invoice $invoice = null)
    {
        ...
        */
        ...
        if (isset($data['covered_period_end_date']) && !$invoice->is_recurring) {
            $invoice->covered_period_end_date = Utils::toSqlDate($data['covered_period_end_date']);
        }

        // Addition (Dylan) : added commitment period frequency on save of the invoice
        if (isset($data['commitment_period_frequency_id']) && $invoice->is_recurring) {
            // If a commitment period is set ( 1 equals none )
            if($data['commitment_period_frequency_id'] > 1) {
                if($invoice->last_commitment_period_date && $invoice->next_commitment_period_date) {
                    // Check if the start date && commitment period frequency hasn't changed
                    $invoice_start_date = new DateTime($invoice->start_date);
                    $input_start_date = new DateTime($data['start_date']);
                    $start_date_is_diff = $invoice_start_date != $input_start_date;

                    if($start_date_is_diff || (
                            $data['commitment_period_frequency_id'] != $invoice->commitment_period_frequency_id
                        )) {
                        $invoice->commitment_period_frequency_id = $data['commitment_period_frequency_id'];
                        $invoice->last_commitment_period_date = $input_start_date->format('Y-m-d');
                        $invoice->next_commitment_period_date = $this->findNextCommitmentFrequencyDate(
                            $invoice->commitment_period_frequency_id,
                            $invoice->last_commitment_period_date
                        );
                    }
                    ...
                    $invoice->commitment_period_frequency_id = $data['commitment_period_frequency_id'];
                    $invoice->last_commitment_period_date = $start_date->format('Y-m-d');
                    $invoice->next_commitment_period_date = $this->findNextCommitmentFrequencyDate(
                        $invoice->commitment_period_frequency_id,
                        $invoice->last_commitment_period_date
                    );
                ...
                $invoice->commitment_period_frequency_id = 1;
                $invoice->last_commitment_period_date = null;
                $invoice->next_commitment_period_date = null;
            }
        }
    ...
    /**
     * Addition (Dylan) : Added a parameter if the function is called from updateInvitations function
     *
     * @param $invoice
     * @param bool $new_client
     * @return mixed
     * @see updateInvitations()
     */
    private function saveInvitations($invoice, $new_client = false)
    {
        // Addition (Dylan) : Adapt client if he is changed in invoice
        $client = $new_client ?: $invoice->client;
    ...
    /**
     * Addition (Dylan) : Update invitations links on invoice client change
     *
     * @param $invoice
     * @param $newClientId
     */
    public function updateInvitations($invoice, $newClientId)
    {
        $invitations = $invoice->invitations;
        foreach ($invitations as $invitation) {
            Invitation::where('invoice_id','=',$invoice->id)->first()->forceDelete();
        }
        $new_client = Client::where('id',$newClientId)->first();
        $invoice->client_id = $newClientId;
        $invoice->save();
        $this->saveInvitations($invoice, $new_client);
    }

    /**
     * Addition (Dylan) : Permanently delete invoice
     *
     */
    public function perm_delete(Invoice $invoice)
    {
        $invoice->forceDelete();
    }
    ...
    public function findInvoiceByInvitation($invitationKey)
    {
        ...
        // Addition (Dylan) : Added payments in invoice object
        $invoice->load('user', 'invoice_items', 'documents', 'invoice_design', 'account.country', 'client.contacts', 'client.country', 'payments');
    ...
    public function createRecurringInvoice(Invoice $recurInvoice)
    {
        ...
        // Addition (Dylan) : Add covered period end date to invoice object
        if($recurInvoice->last_sent_date) {
            $invoice->covered_period_end_date = $this->findCoveredPeriodEndDate(
                $recurInvoice->frequency_id,
                date('Y-m-d')
            );
        } else {
            $invoice->covered_period_end_date = $this->findCoveredPeriodEndDate(
        ...
        // If the recurring invoice have a commitment period
        if ($recurInvoice->next_commitment_period_date != null) {
            $next_commitment_period_date = new DateTime($recurInvoice->next_commitment_period_date);
            $invoice_date = new DateTime($invoice->invoice_date);
            if ($invoice_date >= $next_commitment_period_date) {
                $recurInvoice->last_commitment_period_date = $invoice_date->format('Y-m-d');
                $recurInvoice->next_commitment_period_date = $this->findNextCommitmentFrequencyDate(
                    $recurInvoice->commitment_period_frequency_id,
        ...
        // Addition (Dylan) : last/next_commitment_period_date and commitment_period_frequency_id added on creation
        $invoice->last_commitment_period_date = $recurInvoice->last_commitment_period_date;
        $invoice->next_commitment_period_date = $recurInvoice->next_commitment_period_date;
        $invoice->commitment_period_frequency_id = $recurInvoice->commitment_period_frequency_id;
    ...
    /**
     * Addition (Dylan) : Find client first invoice date
     *
     * @return false|string|null
     */
    public function getClientFirstInvoiceDate($clientid)
    {
        return Invoice::select('invoice_date')
    ...
    /**
     * Addition (Dylan) : Find invoices between start and end date
     *
     * @param $start_of_month
     * @param $end_of_month
     * @return false|string|null
     */
    public function findAllBetweenDates($start_of_month, $end_of_month)
    {
        return Invoice::whereBetween('invoice_date', [$start_of_month,$end_of_month])
            ->where('invoice_type_id', 1)
            ->where('is_recurring', 0)
            ->withTrashed()
            ->get();
    ...
    /**
     * Addition (Dylan) : Find all recurring created invoices between start and end date
     *
     * @param $start_of_month
     * @param $end_of_month
     * @return false|string|null
     */
    public function findAllRecurringBetweenDates($start_of_month, $end_of_month)
    {
        return Invoice::whereBetween('invoice_date', [$start_of_month,$end_of_month])
            ->where('invoice_type_id', 1)
            ->where('is_recurring', 0)
    ...
    /**
     * Addition (Dylan) : Get yearly invoice total amount of invoices
     *
     * @param $year
     * @return false|string|null
     */
    public function getYearlyInvoiceTotal($year)
    {
        return Invoice::whereBetween('invoice_date', ["$year-01-01","$year-12-31"])
            ->where('invoice_type_id', 1)
            ->where('is_recurring', 0)
            ->withTrashed()
            ->sum('amount');
    ...
    /**
     * Addition (Dylan) : Get all yearly clients
     *
     * @param $year
     * @return false|string|null
     */
    public function getAllYearlyClients($year)
    {
        return DB::table('invoices')
            ->select('clients.id')
            ->join('clients', 'clients.id', '=', 'invoices.client_id')
            ->whereBetween('invoices.invoice_date', ["$year-01-01","$year-12-31"])
    ...
    /**
     * Addition (Dylan) : Find the next covered period date with the frequency and the start date
     * @param $frequency_id
     * @param $start_date
     * @return false|string|null
     */
    public function findCoveredPeriodEndDate($frequency_id, $start_date)
    {
        $next_date = null;
        switch ($frequency_id) {
            case "1":
                ...
                break;
            case "2":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 14 days'));
                break;
            case "3":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 28 days'));
                break;
            case "4":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 1 month'));
                break;
            case "5":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 2 months'));
                break;
            case "6":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 3 months'));
                break;
            case "7":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 4 months'));
                break;
            case "8":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 6 months'));
                break;
            case "9":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 1 year'));
                break;
            case "10":
                $next_date = date('Y-m-d', strtotime($start_date. ' + 2 years'));
                break;
        }
    ...
    /**
     * Addition (Dylan) : Find the next commitment period date with the frequency and the last date
     * @param $commitment_period_frequency_id
     * @param $last_commitment_period_date
     * @return false|string|null
     */
    public function findNextCommitmentFrequencyDate($commitment_period_frequency_id, $last_commitment_period_date)
    {
        $next_date = null;
        $last_commitment_period_date = new DateTime($last_commitment_period_date);
        switch ($commitment_period_frequency_id) {
            case "2":
                $next_date = $last_commitment_period_date->modify('first day of +1 year');
                break;
            case "3":
                $next_date = $last_commitment_period_date->modify('first day of +6 months');
                break;
            case "4":
                $next_date = $last_commitment_period_date->modify('first day of +3 months');
                break;
        }
