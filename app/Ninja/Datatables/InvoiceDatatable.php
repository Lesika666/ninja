...
class InvoiceDatatable extends EntityDatatable
{
    ...
    public function columns()
    {
        ...
        return [
            ...
            // Addition (Dylan) : Invoice articles labels
            [
                'products',
                function ($model) {
                    return $this->showWithTooltip($model->invoice_product_key);
                },
            ],
            // Removed (Dylan) : Remove "due_date" / "valid_until" column from invoice list
//            [
...
//            ],
    ...
    public function actions()
    {
        ...
        return [
            // Addition (Dylan) : Add an option to download invoice directly from datatable
            [
                trans('texts.download_'.$entityType),
                function ($model) use ($entityType) {
                    return "javascript:submitForm_{$entityType}('download', {$model->public_id})";
                },
                function ($model) use ($entityType) {
                    return $entityType == ENTITY_INVOICE && Auth::user()->can('edit', [ENTITY_INVOICE, $model]);
                },
            ],
            [
    ...
    private function getStatusLabel($model)
    {
        ...
        $label = Invoice::calcStatusLabel($model->invoice_status_name, $class, $this->entityType, $model->quote_invoice_id, $model->partial_due_date ?: $model->due_date_sql, $model->is_recurring);
