...
class InvoiceController extends BaseController
{
    ...
    {
        // Addition (Dylan) : Added stats data
        $invoices = Invoice::getAllUnpaidInvoices();
        $allUnpaidInvoice['count'] = count($invoices);
        $allUnpaidInvoice['balance'] = 0;
        foreach ($invoices as $invoice) {
            $allUnpaidInvoice['balance'] += floatval($invoice->balance);
        }

        $invoices = Invoice::getFullyUnpaidInvoices();
        $fullyUnpaidInvoice['count'] = count($invoices);
        $fullyUnpaidInvoice['balance'] = 0;
        foreach ($invoices as $invoice) {
            $fullyUnpaidInvoice['balance'] += floatval($invoice->balance);
        }

        $invoices = Invoice::getLateUnpaidInvoices();
        $lateUnpaidInvoice['count'] = count($invoices);
        $lateUnpaidInvoice['balance'] = 0;
        foreach ($invoices as $invoice) {
            $lateUnpaidInvoice['balance'] += floatval($invoice->balance);
        }

        $invoices = Invoice::getPartiallyPaidInvoices();
        $partiallyPaidInvoice['count'] = count($invoices);
        $partiallyPaidInvoice['balance'] = 0;
        foreach ($invoices as $invoice) {
            $partiallyPaidInvoice['balance'] += floatval($invoice->balance);
        }
        ...
        $data = [
            ...
            'allUnpaidInvoice' => $allUnpaidInvoice,
            'fullyUnpaidInvoice' => $fullyUnpaidInvoice,
            'lateUnpaidInvoice' => $lateUnpaidInvoice,
    ...
    public function edit(InvoiceRequest $request, $publicId, $clone = false)
    {
        ...
        // Addition (Dylan) : Store all client list
        ...
        // Addition (Dylan) : added covered period end date on invoice object
        $invoice->covered_period_end_date = Utils::fromSqlDate($invoice->covered_period_end_date);
        // Addition (Dylan) : added covered period end date on invoice object
        $invoice->last_commitment_period_date = Utils::fromSqlDate($invoice->last_commitment_period_date);
        $invoice->next_commitment_period_date = Utils::fromSqlDate($invoice->next_commitment_period_date);
        ...
        // Addition (Dylan) : added payment method name on invoice object
        if (count($invoice->payments)) {
            $invoice->payments->each(function ($payment) {
                $payment_type = PaymentType::find($payment->payment_type_id);
                $payment->payment_type = $payment_type ? trans("texts.payment_type_$payment_type->name") : '';
            });
        }
        ...
        foreach ($allClients as $client) {
            ...
            $content = $client->name.' • '.$contact->first_name.' '.$contact->last_name.' - '.$contact->email;
            if (array_key_exists($client->id,$clientList)) {
//                var_dump("true");die;
            }
            array_push($clientList, [$client->id => $content]);
        }
    ...
    public function update(UpdateInvoiceRequest $request)
    {
        ...
        // Addition (Dylan) : Invoice's client update
        if (isset($data['updateInvoiceClient']) && !empty($data['updateInvoiceClient'])) {
            if ($data['updateInvoiceClient'] != $invoice->client_id) {
                $this->invoiceRepo->updateInvitations($invoice, $newClientId = $data['updateInvoiceClient']);
            }
        }
