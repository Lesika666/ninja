...
class ProposalController extends BaseController
{
    ...
    public function create(ProposalRequest $request)
    {
        $data = array_merge($this->getViewmodel(), [
            ...
            // Addition (Dylan) : Change orderBy parameters to place newest invoices on top
//            'invoices' => Invoice::scope()->with('client.contacts', 'client.country')->unapprovedQuotes()->orderBy('id')->get(),
            'invoices' => Invoice::scope()->with('client.contacts', 'client.country')->unapprovedQuotes()->orderBy('id', 'DESC')->get(),
            'invoicePublicId' => $request->invoice_id,
            'templatePublicId' => $request->proposal_template_id,
        ]);

        // Addition (Dylan) : Add invoice items to invoice object
        foreach ($data['invoices'] as $invoice) {
            $invoice->setAttribute('invoice_items', $invoice->invoice_items()->get());
        }
