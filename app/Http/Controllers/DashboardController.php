...
{
    public function __construct(DashboardRepository $dashboardRepo, ChartService $chartService)
    {
        // Addition (Dylan) : Added invoiceItemService, chartService
...
//        $upcoming = $dashboardRepo->upcoming($accountId, $userId, $viewAll);
        // Addition (Dylan) : Filter upcoming invoice with only recurring ones
        $upcomingResults = $dashboardRepo->upcomingRecurring();
        $upcoming = $upcomingResults['collection'];
        $upcomingTotal = $upcomingResults['total'];
    ...
    public function chartData($startDate, $endDate)
    {
        $chartService = $this->chartService;

        $data = $this->dashboardRepo->statsBuilder($startDate, $endDate, $chartService);
