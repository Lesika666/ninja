    ...
    public function handle()
    {
        ...
        try {
            ...
            if (! $pdf && ($key = env('PHANTOMJS_CLOUD_KEY'))) {
                // Addition (Dylan) : Modify api request to include renderSettings options (https://phantomjscloud.com/docs/http-api/interfaces/irendersettings.html)
                $url = "http://api.phantomjscloud.com/api/browser/v2/{$key}/?request=%7Burl:%22{$link}?phantomjs=true%26phantomjs_secret={$phantomjsSecret}%22,renderType:%22pdf%22%2CrenderSettings%3A%7BpdfOptions%3A%7Bformat%3A%22A4%22%2Clandscape%3Atrue%7D%7D%7D";
