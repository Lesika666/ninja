...
NINJA.decodeJavascript = function(invoice, javascript)
{
    ...
    var json = {
        ...
        // Addition (Dylan) : Added payments to invoice rendering
        'invoiceLinePayments': NINJA.invoicePayments(invoice),
        'invoiceLinePaymentColumns': NINJA.invoicePaymentColumns(invoice, javascript),
        // Addition (Dylan) : Added commitment period to invoice rendering
        'commitmentPeriod': NINJA.commitmentPeriod(invoice),
...
// Addition (Dylan) : payments table columns width
NINJA.invoicePaymentColumns = function(invoice, design)
{
    var columns = [];
    var fields = NINJA.paymentFields();

    for (var i=0; i<fields.length; i++) {
        var field = fields[i];
        var width = 0;

        if (field == 'payment.created_at') {
            width = 120
        } else if (field == 'payment.payment_type') {
            width = 100
        } else if (field == 'payment.transaction_reference') {
            width = 210
        } else if (field == 'payment.amount') {
            width = 100
        } else {
            width = 100;
        }
...
// Addition (Dylan) : payments field to the invoice table
NINJA.paymentFields = function() {
    return fields = [
        'payment.created_at',
...
// Addition (Dylan) : payments lines to the invoice
NINJA.invoicePayments = function(invoice) {

    var grid = [[]];
    var styles = ['tableHeader'];

    if(!invoice.payments.length) {
        var field = invoiceLabels.no_payments;
        var headerStyles = styles.concat([snakeToCamel(field), snakeToCamel(field) + 'TableHeader']);
        headerStyles.push('firstColumn');
        headerStyles.push('lastColumn');
        grid[0].push({text: field, style: headerStyles});
        return NINJA.prepareDataTable(grid, 'invoicePayments');
    }

    var fields = NINJA.paymentFields();

    for (var i=0; i<fields.length; i++) {
        var field = fields[i].split('.')[1];
        ...
        if (field == 'created_at') {
            grid[0].push({text: invoiceLabels.payment_date, style: headerStyles});
        } else if (field == 'payment_type') {
            grid[0].push({text: invoiceLabels.method, style: headerStyles});
        } else if (field == 'transaction_reference') {
            grid[0].push({text: invoiceLabels.reference, style: headerStyles});
        } else if (field == 'amount') {
            grid[0].push({text: invoiceLabels.amount, style: headerStyles});
        }
    }
    ...
    for (var i=0; i<invoice.payments.length; i++) {
        ...
        var transactionReference = payment.transaction_reference;
        var amount = NINJA.parseFloat(payment.amount) ? formatMoneyInvoice(NINJA.parseFloat(payment.amount), invoice, null, getPrecision(NINJA.parseFloat(payment.amount))) : ' ';
        ...
        for (var j=0; j<fields.length; j++) {
            ...
            if (field == 'created_at') {
                value = createdAt;
                styles.push('created_at');
            } else if (field == 'payment_type') {
                value = paymentType;
                styles.push('payment_type');
            } else if (field == 'transaction_reference') {
                value = transactionReference;
                styles.push('transaction_reference');
            } else if (field == 'amount') {
                value = amount;
                styles.push('amount');
            }
...
// Addition (Dylan) : commitment period to the invoice
NINJA.commitmentPeriod = function(invoice) {
    if (invoice.last_commitment_period_date &&
        invoice.next_commitment_period_date &&
        invoice.commitment_period_frequency_id &&
        invoice.covered_period_end_date) {
        return invoiceLabels.commitment_period+" : "+invoice.last_commitment_period_date+" - "+invoice.next_commitment_period_date;
    }
    return "";
}
...
NINJA.invoiceDetails = function(invoice) {
    ...
    if (invoice.features.invoice_settings && account.invoice_fields) {
        ...
        // Addition (Dylan) : Add covered period end date to invoice details
        fields.push('invoice.covered_period_end_date');
    } else {
        var fields = [
    ...
    // Addition (Dylan) : Removed due date
    fields.splice(fields.indexOf('invoice.due_date'), 1);
...
NINJA.renderField = function(invoice, field, twoColumn) {
    ...
    if (field == 'client.client_name') {
        ...
        // Addition (Dylan) : changed po_number label to quote
        label = invoiceLabels.quote;
    ...
    // Removed (Dylan) : removed balance due information on invoice details
    ...
    // Addition (Dylan) : added rendering of covered_period_end_date on invoice details
    else if (field == 'invoice.covered_period_end_date') {
        if (invoice.covered_period_end_date) {
            label = invoiceLabels.covered_period;
            value = invoice.invoice_date+" - "+invoice.covered_period_end_date;
        }
    } else if (field == '.blank') {
        value = ' ';
    }
