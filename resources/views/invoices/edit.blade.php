...
@section('content')
    ...
    <div class="row" style="min-height:195px" onkeypress="formEnterClick(event)">
    		...
    		@if ($invoice->id || $data)
				<div class="form-group">
					...
					<div class="col-lg-8 col-sm-8">
						...
						{{-- Addition (Dylan) : Add an input to show client list--}}
						| <a id="editInvoiceClient" class="pointer">{{ trans('texts.edit_invoice_client') }}</a>
                        @endcan
						{{-- Addition (Dylan) : Create an input to update invoice client --}}
						<h4 id="editInvoiceClientContainer" class="hidden">
							{!! Former::select('updateInvoiceClient')
								->label('')
								->options($clientList, $invoice->client->id)
								!!}
						</h4>
					</div>
				</div>
		...
		<div class="col-md-4" id="col_2">
			<div data-bind="visible: !is_recurring()">
				...
				<!-- Addition (Dylan) : Covered period end date input !-->
				@if ($invoice->covered_period_end_date)
					{!! Former::text('covered_period_end_date')->data_bind("datePicker: covered_period_end_date, valueUpdate: 'afterkeydown'")->label('covered_period_end_date')
							->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT, DEFAULT_DATE_PICKER_FORMAT))->appendIcon('calendar')->addGroupClass('covered_period_end_date') !!}
				@endif
			...
			<div data-bind="visible: is_recurring" style="display: none">
				...
				<!-- Addition (Dylan) : Commitment period input !-->
				{!! Former::select('commitment_period_frequency_id')->label(trans('texts.commitment_period'))->options([
					'1' => trans('texts.none'),
					'2' => trans('texts.freq_annually'),
					'3' => trans('texts.freq_six_months'),
					'4' => trans('texts.freq_three_months'),
				])->data_bind("value: commitment_period_frequency_id")->addGroupClass('commitment_period_frequency_id') !!}
			</div>
            @endif
                ...
                <div class="col-lg-8 col-sm-8 col-sm-offset-4 smaller" style="padding-top: 10px;">
    				...
    				@elseif ($invoice->id)
                        @if (isset($lastSent) && $lastSent)
							{{-- Addition (Dylan) : Added last send date edit btn --}}
							<div>
								{!! trans('texts.last_sent_on', ['date' => link_to('/invoices/'.$lastSent->public_id, $invoice->last_sent_date, ['id' => 'lastSent'])]) !!}
								<span id="editLastSendDate" class="glyphicon glyphicon-edit" style="padding-left:10px;color:#B1B5BA"></span>
								<div id="editLastSendDateContainer" class="hidden">
									<input type="date" name="last_sent_date">
								</div>
							</div>
	...
	// Addition (Dylan) : Add permanent remove option
	function onPermDeleteClick() {
		sweetConfirm(function() {
			submitBulkAction('perm_delete');
		});
	}
	...
	{{-- Addition (Dylan) : Added JS to hide/show update btns --}}
	<script type="text/javascript">
		$(function() {
			$('#editInvoiceClient').click(function() {
				$('#editInvoiceClientContainer').hasClass('hidden') ?
						$('#editInvoiceClientContainer').removeClass('hidden') :
						$('#editInvoiceClientContainer').addClass('hidden');
			})

			$('#editLastSendDate').click(function() {
				$('#editLastSendDateContainer').hasClass('hidden') ?
						$('#editLastSendDateContainer').removeClass('hidden') :
						$('#editLastSendDateContainer').addClass('hidden');
			})
		});
	</script>
